    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if role not in ['user', 'driver']:
                return Response({'error': 'Invalid role. Must be "user" or "driver".'}, status=status.HTTP_400_BAD_REQUEST)
...
def send_otp_login(request):
    ...
    if serializer.is_valid():
        ...
        if not user.is_active or not user.is_verified:
            return Response({'error': 'User is not active or verified. Please active or verified first'}, status=status.HTTP_403_FORBIDDEN)
...
def verify_otp(request):
    ...
    if serializer.is_valid():
        ...
        if user.otp != otp or user.role != role:
            return Response({'error': 'Invalid OTP or role'}, status=status.HTTP_400_BAD_REQUEST)
        if user.otp_expired and timezone.now() > user.otp_expired:
            return Response({'error': 'OTP expired'}, status=status.HTTP_400_BAD_REQUEST)
